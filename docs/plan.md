...
- Next.js frontend now includes login/register, a signed-in status widget with refresh/logout, and a `/menus` dashboard that lists menus and lets you create/delete courses/items (media IDs are still pasted manually until search UI ships).

## Near-Term Priorities
1. **Frontend flows:** finish the search/ingest drawer, teach `/menus` to browse the catalog instead of pasting IDs, and add read-only public menu pages. _Login/register + session widgets and the manual menu editor are live; next step is wiring API-powered search results and slug pages._
