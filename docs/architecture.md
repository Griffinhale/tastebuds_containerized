...
- **Search:** `GET /api/search` queries Postgres first, optionally fans out to external connectors, and requires auth for any external sources. External results are stored in a short-TTL preview cache (with payload/metadata caps); preview detail views are read-only and full ingest happens only after an authenticated user explicitly ingests or saves to a menu/library.
