...
- **Search:** `GET /api/search` queries Postgres first using a persisted `media_items.search_vector` (GIN indexed, `english_unaccent` config) with triggers that refresh vectors on media + extension updates; non-Postgres engines (e.g., SQLite in CI) use normalized substring matching for internal search. External fan-out requires auth and results are stored in a short-TTL preview cache (with payload/metadata caps); preview detail views are read-only and full ingest happens only after an authenticated user explicitly ingests or saves to a menu/library.
