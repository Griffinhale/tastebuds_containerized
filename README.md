...
- Integrations are now manageable via `/api/integrations` and the `/integrations` UI: Spotify OAuth linking with menu export, Arr webhook intake with an ingest queue, and Jellyfin/Plex sync adapters that ingest TMDB-backed library items.
